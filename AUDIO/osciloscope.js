...
var renderers={'r0':(function(){var barsArr=[],initialized=false,barsEl;
	var height=0;
	var init=function(config){var count=config.count;
		...
		for(var i=0;
			i<count;
			i++){var nunode=document.createElement('div');
	...
	var max=256;
	var renderFrame=function(frequencyData){for(var i=0;
		i<barsArr.length;
		i++){var bar=barsArr[i];
			bar.style.height=((frequencyData[i]/max)*height+'px');
		}};
		return{init:init,isInitialized:function(){return initialized;
		},renderFrame:renderFrame}})(),'r1':(function(){var barsArr=[],initialized=false,barsEl;
			var height=0;
			var init=function(config){var count=config.count;
				var width=config.width;
				var barWidth=(width/count)>>0;
			...
			var renderFrame=function(frequencyData){for(var i=0;
				...
				return{init:init,isInitialized:function(){return initialized;
				},renderFrame:renderFrame}})(),'r2':(function(){var barsArrLeft=[],barsArrRight=[],initialized=false,barsEl;
					var height=0;
					var width=0;
					var init=function(config){var count=config.count;
						width=config.width;
					...
					var renderFrame=function(frequencyData){for(var i=0;
						...
						return{init:init,isInitialized:function(){return initialized;
							...
							var renderFrame=function(frequencyData){for(var i=0;
								i<circles.length;
								i++){var circle=circles[i];
									circle.style.cssText='-webkit-transform:scale('+((frequencyData[i]/max))+')';
								}};
								return{init:init,isInitialized:function(){return initialized;
								},renderFrame:renderFrame}})(),'r4':(function(){var bars=[];
									var initialized=false;
									var height=0;
									var width=0;
									var init=function(config){var count=config.count;
										...
										for(var i=0;
											i<twopi;
											i+=change){var node=document.createElement('div');
									...
									var renderFrame=function(frequencyData){for(var i=0;
										...
										return{init:init,isInitialized:function(){return initialized;
										},renderFrame:renderFrame}})(),'r5':(function(){var barsRight=[];
											var barsLeft=[];
											var initialized=false;
											var height=0;
											var width=0;
											...
											var renderFrame=function(frequencyData){for(var i=0;
												i<barsRight.length;
												i++){var bar=barsRight[i];
													var left=barsLeft[i];
													bar.style.height=left.style.height=((frequencyData[barsRight.length-i]/max)*150)+'px';
												}};
												return{init:init,isInitialized:function(){return initialized;
												},renderFrame:renderFrame}})()};
												window.onload=function(){function Visualization(config){var audio,audioStream,analyser,source,audioCtx,canvasCtx,frequencyData,running=false,renderer=config.renderer,width=config.width||360,height=config.height||360;
													var init=function(){audio=document.getElementById('r0audio');
												...
												this.start=function(){audio.play();
													running=true;
													renderFrame();
												};
												...
												this.setRenderer=function(r){if(!r.isInitialized()){r.init({count:analyser.frequencyBinCount,width:width,height:height});
											}
											renderer=r;
										};
										this.isPlaying=function(){return running;
										}
										var renderFrame=function(){analyser.getByteFrequencyData(frequencyData);
										...
										for(var i=0;
											i<vis.length;
											i++){vis[i].onclick=(function(){return function(){var el=this;
												var id=el.parentNode.id;
												if(!v){v=new Visualization({renderer:renderers[id]});
											}
											v.setRenderer(renderers[id]);
											if(v.isPlaying()){if(lastElparentId===id){v.stop();
												el.style.backgroundColor='rgba(0,0,0,0.5)';
											}else{lastEl.style.backgroundColor='rgba(0,0,0,0.5)';
